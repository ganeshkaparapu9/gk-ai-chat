    ...
    <div className="flex flex-col h-screen bg-background">
      ...
      <header className="sticky top-0 z-10 backdrop-blur-xl bg-background/80 border-b border-border px-6 py-4">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-semibold text-foreground tracking-tight">AI Chat</h1>
          ...
          <div className="flex items-center gap-3">
            {isLoading ? (
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-accent-muted">
                ...
                <span className="text-xs font-medium text-accent">{statusMessage}</span>
              </div>
            ) : (
      ...
      <main className="flex-1 overflow-y-auto px-4 py-6">
        <div className="max-w-3xl mx-auto space-y-5">
          {error && (
            <div className="rounded-3xl border border-red-500 bg-red-500/10 px-4 py-3 text-sm text-red-700 shadow-sm">
              <span className="font-medium">Error:</span> {error.message}
            </div>
          )}
          {isLoading && (
            <div className="rounded-3xl border border-border bg-surface px-4 py-3 text-sm text-muted shadow-sm">
              <span className="font-medium text-foreground/80">Status:</span> {statusMessage}
            </div>
          )}
          {/* Empty state */}
          {messages.length === 0 && (
